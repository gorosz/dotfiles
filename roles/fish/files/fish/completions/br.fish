...
complete -c br -n "__fish_use_subcommand" -s c -l cmd -d 'commands to execute (space separated, experimental)'
complete -c br -n "__fish_use_subcommand" -s o -l out -d 'where to write the produced path (if any)'
complete -c br -n "__fish_use_subcommand" -s g -l gitignore -d 'respect .gitignore rules (yes, no, auto)'
...
complete -c br -n "__fish_use_subcommand" -l no-style -d 'whether to remove all style and colors'
